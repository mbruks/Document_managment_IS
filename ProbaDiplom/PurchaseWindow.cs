    ...
    public partial class PurchaseWindow : Form
    {
        ...
        private void dgvDataOrders_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                PriceButton.Text = dgvDataOrders.Rows[e.RowIndex].Cells["cost_pur"].Value.ToString();
        ...
        private void safeButton_Click(object sender, EventArgs e)
        {
            ...
            if (rowIndex < 0) // insert
            {
                ...
                {
                    conn.Open();
                    sql = @"SELECT * from purchase_insert(:_number, :_product, :_volume, :_cost_pur, :_all_cost, :_date)";
                    cmd = new NpgsqlCommand(sql, conn);
                    ...
                    cmd.Parameters.AddWithValue("_cost_pur", int.Parse(PriceButton.Text));
            ...
            else //update
            {
                ...
                {
                    conn.Open();
                    sql = @"SELECT *  from purchase_update(:_id_purchase, :_number, :_product, :_volume, :_cost_pur, :_all_cost, :_date)";
                    cmd = new NpgsqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("_id_purchase", int.Parse(dgvDataOrders.Rows[rowIndex].Cells["id_purchase"].Value.ToString()));
                    cmd.Parameters.AddWithValue("_number", numberButton.Text);
                    cmd.Parameters.AddWithValue("_product", ProductcomboBox.SelectedValue);
                    cmd.Parameters.AddWithValue("_volume", int.Parse(KolvoButton.Text));
                    cmd.Parameters.AddWithValue("_cost_pur", int.Parse(PriceButton.Text));
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            if (rowIndex < 0)
            {
                MessageBox.Show("Выберите закупку которую хотите удалить!");
                return;
            }
            try
            {
                conn.Open();
                sql = @"select * from purchase_delete(:_id_purchase)";
                cmd = new NpgsqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("_id_purchase", int.Parse(dgvDataOrders.Rows[rowIndex].Cells["id_purchase"].Value.ToString()));
        ...
        private void SearchButton_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                sql = $"SELECT * FROM public.purchase WHERE number = '{search}'";
                cmd = new NpgsqlCommand(sql, conn);
                dt = new System.Data.DataTable();
                dt.Load(cmd.ExecuteReader());
                dgvDataOrders.DataSource = null; // reset datagridiew
        ...
        private void SortedButton_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                sql = $"SELECT * FROM public.purchase WHERE date BETWEEN '{FROMdate}' AND '{TOdate}'";
                cmd = new NpgsqlCommand(sql, conn);
                dt = new System.Data.DataTable();
                dt.Load(cmd.ExecuteReader());
                dgvDataOrders.DataSource = null; // reset datagridiew
        ...
        public int A()
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand($"SELECT COUNT(*) FROM public.purchase WHERE number = '{schet}'", conn))
                {
        ...
        public int B()
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand($"SELECT SUM(all_cost) FROM public.purchase WHERE number = '{schet}'", conn))
                {
        ...
        private void OrderButton_Click(object sender, EventArgs e)
        {
            try
            {
                var schet = NumberTextBox.Text;
                conn.Open();
                command = $"SELECT name, volume, cost_pur, all_cost FROM public.purchase FULL JOIN product ON public.purchase.product = product.id_product WHERE number = '{schet}'";
        ...
        private void ExportToWord(System.Data.DataTable dataTable)
        {
            if (dataTable.Rows.Count > 0)
            {
                ...
                para1.Range.set_Style(styleHeading1);
                para1.Range.Text = "Накладная на закупку";
                ...
                para1.Range.Text = "Накладная №" + NumberTextBox.Text;
                para1.Range.InsertParagraphAfter();


                //Заголовок документа
                word.Documents.Application.Caption = "Накладная на закупку";
